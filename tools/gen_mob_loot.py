...
def load_known_mobs():
    ...
    known = {}
    for slug in re.findall(r'//\s*"([a-z_]+)"', open(hpp).read()):
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "GeneratedMobLoot.hpp"), "w") as f:
...
""")
    ...
    with open(os.path.join(OUT_DIR, "GeneratedMobLoot.cpp"), "w") as f:
...
""")
        for enum_name, mob, entries in tables:
            ...
            if not entries:
                f.write("        // (no supported entries)\n")
            f.write("    };\n\n")

        f.write("    const MobLootTable kMobLootTables[] = {\n")
        for enum_name, mob, entries in tables:
            f.write(f"        {{ EntityTypeId::{enum_name}, k_{mob}, {len(entries)} }},\n")
        f.write("    };\n\n")
