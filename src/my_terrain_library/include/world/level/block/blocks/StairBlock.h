...
public:
    ...
    explicit StairBlock(const Properties& properties) : Block(properties) {
        ...
        // NOTE: C++ virtual dispatch doesn't work in base constructors, so
        // Block's constructor called the base createBlockStateDefinition().
        // We need to rebuild the state definition now that our override is active.
