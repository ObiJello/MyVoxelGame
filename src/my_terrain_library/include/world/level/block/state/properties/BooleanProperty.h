...
public:
     ...
     *
     * IMPORTANT: Override needed because std::vector<bool> is a specialization
     * that returns proxy objects instead of bool references. We must explicitly
     * convert to bool before storing in std::any to ensure proper type storage.
     */
    std::any getValueAtIndex(size_t index) const override {
        if (index < 2) {
            // Explicitly convert to bool to avoid std::vector<bool> proxy issues
            bool value = (index == TRUE_INDEX);
