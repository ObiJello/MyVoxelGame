...
private:
    ...
    int m_maxY;
    int m_height;
    std::vector<BlockState*> m_blocks;  // Flat array: index = (y - minY) * 256 + z * 16 + x
...
public:
    ...
    {
        m_height = maxY - minY;
        m_blocks.resize(16 * m_height * 16, Minecraftminecraft::world::level::block::Blocks::AIR->defaultBlockState());
    }
    ...
    BlockState* getBlockState(const BlockPos& pos) override {
    ...
    BlockState* getBlockState(int localX, int y, int localZ) override {
        if (localX < 0 || localX >= 16 || localZ < 0 || localZ >= 16) {
            return Minecraftminecraft::world::level::block::Blocks::AIR->defaultBlockState();
        }
    ...
    BlockState* setBlockState(const BlockPos& pos, BlockState* state, bool moved) override {
    ...
    BlockState* setBlockState(int localX, int y, int localZ, BlockState* state, bool moved) override {
        ...
        int index = getIndex(localX, y, localZ);
        BlockState* oldState = m_blocks[index];
