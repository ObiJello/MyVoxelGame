...
public:
     ...
     */
    bool update(int32_t localX, int32_t localY, int32_t localZ, const BlockState* state) {
        ...
        // Reference: Heightmap.java lines 92-105
            ...
            if (m_blockGetter) {
                for (int32_t y = localY - 1; y >= m_minY; --y) {
                    const BlockState* blockBelow = m_blockGetter(localX, y, localZ);
    ...
    /**
     * Prime heightmaps for a chunk by scanning all columns top-down
     * Reference: Heightmap.java lines 43-80
     *
     * This initializes heightmap values by scanning each column from the top
     * of the world down until an opaque block is found for each heightmap type.
     *
     * @param chunk - The chunk to prime
     * @param types - Set of heightmap types to prime
     */
    static void primeHeightmaps(world::IChunk* chunk, const std::set<Types>& types);

    /**
     * Get the opaque predicate for this heightmap (for use by primeHeightmaps)
     */
    const OpaquePredicate& getIsOpaque() const { return m_isOpaque; }
};
