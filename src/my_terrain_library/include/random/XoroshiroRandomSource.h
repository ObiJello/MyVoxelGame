...
public:
    ...
    /**
     * Set internal state directly (for restoring state after tracing).
     * Similar to setSeed() but with explicit seedLo/seedHi instead of single seed.
     * Note: This is not a Java method - added for trace state restoration.
     */
    void setStateDirectly(uint64_t seedLo, uint64_t seedHi);
