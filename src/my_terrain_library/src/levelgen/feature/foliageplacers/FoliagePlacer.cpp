...
bool FoliagePlacer::tryPlaceLeaf(
    FoliageSetter& foliageSetter,
    WorldgenRandom& random,
    std::shared_ptr<stateproviders::BlockStateProvider> foliageProvider,
...
// Reference: Direction.java getAxisDirection() == POSITIVE
static bool isPositiveAxisDirection(core::Direction dir) {
    return dir == core::Direction::SOUTH || dir == core::Direction::EAST || dir == core::Direction::UP;
}
...
void FoliagePlacer::placeLeavesRowWithHangingLeavesBelow(
    ...
    // Reference: FoliagePlacer.java placeLeavesRowWithHangingLeavesBelow() lines 81-107
    // First place normal leaves row
    placeLeavesRow(foliageSetter, random, foliageProvider, origin, currentRadius, y, doubleTrunk);

    int offset = doubleTrunk ? 1 : 0;
    core::BlockPos logPos = origin.below();
    core::BlockPos::MutableBlockPos pos;

    // Reference: Direction.Plane.HORIZONTAL iteration order: NORTH, EAST, SOUTH, WEST
    static const core::Direction horizontalDirs[] = {
        core::Direction::NORTH, core::Direction::EAST,
        core::Direction::SOUTH, core::Direction::WEST
    };

    for (core::Direction alongEdge : horizontalDirs) {
        core::Direction toEdge = core::rotateYClockwise(alongEdge);
        int offsetToEdge = isPositiveAxisDirection(toEdge) ? currentRadius + offset : currentRadius;
        pos.setWithOffset(origin, 0, y - 1, 0);
        pos.move(toEdge, offsetToEdge);
        pos.move(alongEdge, -currentRadius);

        int offsetAlongEdge = -currentRadius;
        while (offsetAlongEdge < currentRadius + offset) {
            // Check if leaves exist above this position
            bool leavesAbove = foliageSetter.isSet(pos.move(core::Direction::UP));
            pos.move(core::Direction::DOWN);

            // Reference: FoliagePlacer.java tryPlaceExtension() lines 109-115
            // tryPlaceExtension: if dist >= 7 return false (no random consumed)
            //   else if random.nextFloat() > chance return false
            //   else return tryPlaceLeaf(...)
            if (leavesAbove) {
                bool placed = false;
                if (pos.distManhattan(logPos) < 7) {
                    if (!(random.nextFloat() > hangingLeavesChance)) {
                        placed = tryPlaceLeaf(foliageSetter, random, foliageProvider, pos);
                    }
                }
                if (placed) {
                    pos.move(core::Direction::DOWN);
                    // Extension below - tryPlaceExtension again
                    if (pos.distManhattan(logPos) < 7) {
                        if (!(random.nextFloat() > hangingLeavesExtensionChance)) {
                            tryPlaceLeaf(foliageSetter, random, foliageProvider, pos);
                        }
                    }
                    pos.move(core::Direction::UP);
                }
            }
...
int PineFoliagePlacer::foliageRadius(WorldgenRandom& random, int trunkHeight) const {
