...
public:
    ...
    /**
     * Sample with very strong bias toward bottom
     * Reference: VeryBiasedToBottomHeight.java lines 28-39
     */
    template<typename RandomSource>
    int32_t sample(RandomSource& random, const WorldGenerationContext& context) {
        int32_t min = m_minInclusive.resolveY(context);
        ...
        // Uses power of random to heavily bias toward bottom
        float rand1 = random.nextFloat();
        float rand2 = random.nextFloat();
        float biased = 1.0f - rand1 * rand2;
        return min + static_cast<int32_t>(biased * static_cast<float>(max - min - m_inner + 1))
                   + random.nextInt(m_inner + 1);
    }
