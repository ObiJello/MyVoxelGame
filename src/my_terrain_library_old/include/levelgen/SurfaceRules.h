...
public:
     ...
     * @return IBlockType to use, or nullptr if rule doesn't apply
     * Reference: SurfaceRules.java line 782
     */
    virtual ::world::IBlockType* tryApply(int32_t blockX, int32_t blockY, int32_t blockZ) = 0;
};
...
private:
    ::world::IBlockType* m_block;

public:
    StateRule(::world::IBlockType* block) : m_block(block) {}
...
public:
    ...
    ::world::IBlockType* tryApply(int32_t blockX, int32_t blockY, int32_t blockZ) override {
        for (SurfaceRule* rule : m_rules) {
            ::world::IBlockType* block = rule->tryApply(blockX, blockY, blockZ);
            if (block != nullptr) {
                return block;
            }
        }
...
private:
    ::world::IBlockType* m_resultBlock;

public:
    BlockRuleSource(::world::IBlockType* block) : m_resultBlock(block) {}

    ::world::IBlockType* resultBlock() const { return m_resultBlock; }
...
inline RuleSource* state(::world::IBlockType* block) {
...
inline RuleSource* state(const std::string& blockName) {
    return new BlockRuleSource(::world::MinecraftBlocks::get(blockName));
}
