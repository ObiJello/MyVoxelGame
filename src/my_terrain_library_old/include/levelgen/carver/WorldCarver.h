...
protected:
     ...
     */
    ::world::IBlockType* getCarveState(
     ...
     */
    bool canReplaceBlock(const C& configuration, const ::world::IBlockType* block) const;
...
bool WorldCarver<C>::carveBlock(
    ...
    ::world::IBlockType* blockType = chunk->getBlockState(blockPos);
    ...
    ::world::IBlockType* carvedBlock = getCarveState(context, configuration, blockPos, aquifer);
    ...
    if (hasGrass) {
        ...
        ::world::IBlockType* belowBlock = chunk->getBlockState(helperPos);
        if (belowBlock && belowBlock->getIdentifier() == "minecraft:dirt") {
            // Get top material from context (could be grass, podzol, etc.)
            ::world::IBlockType* topBlock = context.topMaterial(biomeGetter, chunk, helperPos,
                                                                  carvedBlock->isFluid());
...
template<typename C>
::world::IBlockType* WorldCarver<C>::getCarveState(
    const CarvingContext& context,
    const C& configuration,
    const core::BlockPos& blockPos,
    Aquifer* aquifer
    ...
    if (aquifer) {
        ...
        ::world::IBlockType* block = aquifer->computeSubstance(singleContext, 0.0);
        ...
        if (block->isAir()) {
            return ::world::MinecraftBlocks::AIR();  // Normal cave - use AIR (matches Java)
        } else if (block->getIdentifier() == "minecraft:water") {
            return ::world::MinecraftBlocks::WATER();  // Underwater cave
        } else if (block->getIdentifier() == "minecraft:lava") {
            return ::world::MinecraftBlocks::LAVA();  // Lava
        }
...
template<typename C>
bool WorldCarver<C>::canReplaceBlock(const C& configuration, const ::world::IBlockType* block) const {
