...
::world::IBlockType* Aquifer::calculate(
...
::world::IBlockType* DisabledAquifer::computeSubstance(
    const density::DensityFunction::FunctionContext& context,
    double density
) {
...
::world::IBlockType* NoiseBasedAquifer::computeSubstance(
    const density::DensityFunction::FunctionContext& context,
    double density
) {
    ...
    if (globalFluid.at(posY) != nullptr && globalFluid.at(posY)->is(::world::MinecraftBlocks::LAVA())) {
        m_shouldScheduleFluidUpdate = false;
        return ::world::MinecraftBlocks::LAVA();
    }
    ...
    ::world::IBlockType* fluidState = closestStatus1.at(posY);
    ...
    if (fluidState != nullptr && fluidState->is(::world::MinecraftBlocks::WATER()) &&
        m_globalFluidPicker->computeFluid(posX, posY - 1, posZ).at(posY - 1) != nullptr &&
        m_globalFluidPicker->computeFluid(posX, posY - 1, posZ).at(posY - 1)->is(::world::MinecraftBlocks::LAVA())) {
...
FluidStatus NoiseBasedAquifer::computeFluid(int32_t x, int32_t y, int32_t z) {
    ...
    for (const auto& offset : SURFACE_SAMPLING_OFFSETS_IN_CHUNKS) {
        ...
        if (topOfAquiferCellPokesAboveSurface || isCenter) {
            ...
            ::world::IBlockType* fluidAtSurface = globalFluidAtSurface.at(adjustedSurfaceLevel);
...
double NoiseBasedAquifer::calculatePressure(
    ...
    int32_t posY = context.blockY();
    ::world::IBlockType* type1 = status1.at(posY);
    ::world::IBlockType* type2 = status2.at(posY);

    // Special case: lava-water boundary always has pressure (Java lines 266-268)
    if ((type1 != nullptr && type1->is(::world::MinecraftBlocks::LAVA()) &&
         type2 != nullptr && type2->is(::world::MinecraftBlocks::WATER())) ||
        (type1 != nullptr && type1->is(::world::MinecraftBlocks::WATER()) &&
         type2 != nullptr && type2->is(::world::MinecraftBlocks::LAVA()))) {
        return 2.0;
    }
...
::world::IBlockType* NoiseBasedAquifer::computeFluidType(
    int32_t x, int32_t y, int32_t z,
    const FluidStatus& globalFluid,
    int32_t fluidSurfaceLevel
) {
    ::world::IBlockType* fluidType = globalFluid.fluidType;
    ...
    if (fluidSurfaceLevel <= -10 &&
        ...
        !globalFluid.fluidType->is(::world::MinecraftBlocks::LAVA())) {
        ...
        if (std::abs(lavaNoiseValue) > 0.3) {
            fluidType = ::world::MinecraftBlocks::LAVA();
        }
    }
