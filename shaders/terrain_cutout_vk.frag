// File: shaders/terrain_cutout_vk.frag (Vulkan twin of terrain_cutout.frag)
// Cutout TERRAIN fragment shader: block_vk.frag plus the greedy-meshing
// tile-rect sample path. Keep in step with the GL terrain_cutout.frag —
// including the inverted-alpha uOverlayColor_ contract (see block.frag).
#version 450
...
// Common UBO (portal pipeline layout, set=1) — see block_vk.frag for the
// std140 prefix rule and the appended uOverlayColor_ field.
layout (std140, set = 1, binding = 0) uniform Common {
    ...
    vec4  uOverlayColor_;  // 352 — see block_vk.frag
} U;
