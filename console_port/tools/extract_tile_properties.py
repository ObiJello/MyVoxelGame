...
def evaluate(cls, args, row):
    ...
    for stmt in re.finditer(r'(?:if\s*\(\s*(!?)\s*(\w+)\s*\)\s*\{?[^;{}]*?)?\b(setTicking|setLightBlock|setLightEmission)\s*\(([^)]*)\)|solid\[id\]\s*=\s*(\w+)', body):
        if stmt[5]:
...
def main():
    ...
    for statement in body.split(';'):
        ...
        for setter, arg in re.findall(r'->\s*(setTicking|setLightBlock|setLightEmission)\s*\(([^)]*)\)', statement[close:]):
    ...
    for tile_id, r in sorted(rows.items()):
        ...
        out.append(f' {{{tile_id},"{r["cls"]}",SurvivalMaterial::{r["material"]},{flag(r["solid"])},'
                   f'{r["lightBlock"]},{r["emission"]},{flag(r["ticking"])},{flag(r["cube"])}}}, // {r["name"]}{note}')
    out += ['};', '}',
