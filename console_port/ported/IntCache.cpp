...
thread_local std::unique_ptr<IntCache::ThreadStorage> IntCache::storage;

void IntCache::CreateNewThreadStorage()
{
	storage = std::make_unique<ThreadStorage>();
	ThreadStorage *tls = storage.get();
...
{
	if (!storage) CreateNewThreadStorage();
	ThreadStorage *tls = storage.get();
