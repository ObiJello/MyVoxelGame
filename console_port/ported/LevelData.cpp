...
LevelData::LevelData()
{
    ...
    m_pGenerator=LevelType::lvl_normal;gameType=GameType::SURVIVAL;
}

LevelData::LevelData(CompoundTag *tag):LevelData()
{
    if(!tag)throw IoError("Null level metadata");
...
LevelData::LevelData(LevelSettings *levelSettings, const wstring& levelName):LevelData()
{
    if(!levelSettings)throw IoError("Null level settings");
...
LevelData::LevelData(LevelData *copy):LevelData()
{
    if(!copy)throw IoError("Null level metadata copy");
    this->seed = copy->seed;
...
{
	if(!gameType)throw IoError("Null game type");
	this->gameType = gameType;

	// 4J Added
	hasBeenInCreative = hasBeenInCreative || (gameType == GameType::CREATIVE) || SavePlatform::cheatsEnabled.load();
}
...
{
	if(!generator)throw IoError("Null generator");
