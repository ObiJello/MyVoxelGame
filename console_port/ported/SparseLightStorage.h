// Native locked CPU port; packed-pointer RCU comments below describe the archive.
#pragma once
#include "xmcore.h"
...
//	unsigned char	planeIndices[128];
	PlaneStorageState dataAndCount;
    static std::recursive_mutex storageMutex;							// Contains packed-together data pointer (lower 48-bits), and count of lines used (upper 16-bits)
...
#endif
public:
    SparseLightStorage(const SparseLightStorage&)=delete;
    SparseLightStorage& operator=(const SparseLightStorage&)=delete;
...
private:
	...
	void updateDataAndCount(PlaneStorageState newDataAndCount);
public:
	int  compress();
