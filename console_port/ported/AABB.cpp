...
thread_local std::unique_ptr<AABB::ThreadStorage> AABB::tls;
AABB::ThreadStorage::ThreadStorage():pool(new AABB[POOL_SIZE]),poolPointer(0) {}
AABB::ThreadStorage::~ThreadStorage() { delete[] pool; }
AABB::ThreadStorage& AABB::storage() {
    if(!tls)tls=std::make_unique<ThreadStorage>();
    return *tls;
}
void AABB::CreateNewThreadStorage() { (void)storage(); }
void AABB::UseDefaultThreadStorage() { (void)storage(); }
void AABB::ReleaseThreadStorage() { tls.reset(); }
