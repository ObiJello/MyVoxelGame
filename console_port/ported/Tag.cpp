...
void Tag::print(const char *prefix, wostream& out)
{
...
{
    if(!dis)throw IoError("Null NBT input");
    NbtReadScope scope;
    ::byte type=dis->readByte();
    NbtReadScope::charge(128,1);
    if(type==TAG_End)return new EndTag();
    if(type>TAG_Int_Array)throw IoError("Unknown NBT tag type");
    wstring name=dis->readUTF();
    NbtReadScope::charge(name.size()*sizeof(wchar_t));
    std::unique_ptr<Tag> tag(newTag(type,name));
    tag->load(dis);
    return tag.release();
}
...
{
	if(!tag || !dos)throw IoError("Null NBT output or tag");
