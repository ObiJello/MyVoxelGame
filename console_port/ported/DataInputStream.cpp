...
{
    if(!in)throw std::invalid_argument("Input stream is null");
}
...
{
	return child().read( b, offset, length );
}
...
{
    return static_cast<::byte>(requiredByte());
}

unsigned char DataInputStream::readUnsignedByte()
{
    return static_cast<unsigned char>(requiredByte());
}
...
{
    return static_cast<wchar_t>(readBits(2));
}
...
{
    if(b.length && !b.data)throw std::out_of_range("Read buffer is null");
...
{
    return std::bit_cast<int>(static_cast<std::uint32_t>(readBits(4)));
}
...
{
    return std::bit_cast<__int64>(readBits(8));
}
...
{
    return std::bit_cast<short>(static_cast<std::uint16_t>(readBits(2)));
}
...
{
    unsigned length=static_cast<unsigned>(readBits(2));
    std::vector<unsigned char> bytes(length);
    for(auto& b:bytes)b=static_cast<unsigned char>(requiredByte());
    std::vector<std::uint16_t> units;
    std::size_t index=0;
    auto next=[&]()->unsigned{if(index>=bytes.size())throw InvalidUtf("Truncated modified UTF character");return bytes[index++];};
    while(index<bytes.size()){unsigned first=next();units.push_back(console::io::decodeUnit(first,next));}
    return console::io::nativeString(units);
}

int DataInputStream::readUTFChar()
{
    int first=child().read();if(first<0)return -1;
    return console::io::decodeUnit(static_cast<unsigned>(first),[&]{return requiredByte();});
}
