// Original plane packing loops with full-width pointer state and native locking.
#include "stdafx.h"
#include "SparseDataStorage.h"
std::recursive_mutex SparseDataStorage::storageMutex;
...
#else
	unsigned char *planeIndices = (unsigned char *)planeAlloc(128 * 128);
#endif
...
SparseDataStorage::SparseDataStorage(bool isUpper)
{
	// Allocate using physical alloc. As this will (by default) return memory from the pool of 4KB pages, the address will in the range of MM_PHYSICAL_4KB_BASE upwards. We can use
	// this fact to identify the allocation later, and so free it with the corresponding call to XPhysicalFree.
	unsigned char *planeIndices = (unsigned char *)planeAlloc(128);
	for( int i = 0; i < 128; i++ )
	{
...
SparseDataStorage::~SparseDataStorage()
{
	unsigned char *indicesAndData = dataAndCount.pointer;
...
{
    std::lock_guard guard(storageMutex);if(!copyFrom)throw IoError("Null sparse storage copy");
	// Extra details of source storage
	PlaneStorageState sourceDataAndCount = copyFrom->dataAndCount;
	unsigned char *sourceIndicesAndData = sourceDataAndCount.pointer;
	int sourceCount = sourceDataAndCount.count;

	// Allocate & copy indices ( 128 bytes ) and any allocated planes (128 * count)
	unsigned char *destIndicesAndData = (unsigned char *)planeAlloc( sourceCount * 128 + 128 );
	...
	dataAndCount = {destIndicesAndData,sourceDataAndCount.count};
...
{
    validateByteRange(dataIn,inOffset,16384);
	...
	//unsigned char *lastDataPointer = dataAndCount.pointer;
	...
	unsigned char *planeIndices = (unsigned char *)planeAlloc(128 * allocatedPlaneCount + 128);
	unsigned char *data = planeIndices + 128;
	...
	PlaneStorageState newDataAndCount{planeIndices,0};
	newDataAndCount.count=allocatedPlaneCount;
...
{
    validateByteRange(retArray,retOffset,16384);
    std::lock_guard guard(storageMutex);
...
{
    planePosition(x,y,z);if(val<0 || val>15)throw IoError("Invalid sparse nibble value");
    std::lock_guard guard(storageMutex);
	unsigned char *planeIndices, *data;
	getPlaneIndicesAndData(&planeIndices, &data);
...
{
    if(!planeRegion(dataIn,x0,y0,z0,x1,y1,z1,offset))return 0;
    std::lock_guard guard(storageMutex);
...
{
    if(!planeRegion(dataInOut,x0,y0,z0,x1,y1,z1,offset))return 0;
    std::lock_guard guard(storageMutex);
...
{
    if(y<0 || y>=128)throw IoError("Invalid sparse plane");
    std::lock_guard guard(storageMutex);
	bool success = false;
	do
	{
		// Get last packed data pointer & count
		PlaneStorageState lastDataAndCount = dataAndCount;

		// Unpack count & data pointer
		int lastLinesUsed = (int)(lastDataAndCount.count);
		unsigned char *lastDataPointer = lastDataAndCount.pointer;
		...
		unsigned char *dataPointer = (unsigned char *)planeAlloc(linesUsed * 128 + 128);
		...
		// Get new data and count packed info
		PlaneStorageState newDataAndCount{dataPointer,0};
		newDataAndCount.count=linesUsed;
		...
		PlaneStorageState lastDataAndCount2 = exchangePlaneState( &dataAndCount, newDataAndCount, lastDataAndCount );
...
void SparseDataStorage::updateDataAndCount(PlaneStorageState newDataAndCount)
{
    std::lock_guard guard(storageMutex);
...
int SparseDataStorage::compress()
{
	...
	unsigned char *planeIndices = lastDataAndCount.pointer;
	unsigned char *data = planeIndices + 128;
	...
	if( needsCompressed )
	{
		unsigned char *newIndicesAndData = (unsigned char *)planeAlloc( 128 + 128 * planesToAlloc );
		...
		// Get new data and count packed info
		PlaneStorageState newDataAndCount{newIndicesAndData,0};
		newDataAndCount.count=planesToAlloc;
...
//			printf("Successfully compressed to %d planes, to delete 0x%x\n", planesToAlloc, planeIndices); 
	...
	{
		return (int)(lastDataAndCount.count);
	}
}
