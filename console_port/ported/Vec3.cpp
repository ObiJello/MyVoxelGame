...
thread_local std::unique_ptr<Vec3::ThreadStorage> Vec3::tls;
Vec3::ThreadStorage::ThreadStorage():pool(new Vec3[POOL_SIZE]),poolPointer(0) {}
Vec3::ThreadStorage::~ThreadStorage() { delete[] pool; }
Vec3::ThreadStorage& Vec3::storage() {
    if(!tls)tls=std::make_unique<ThreadStorage>();
    return *tls;
}
void Vec3::CreateNewThreadStorage() { (void)storage(); }
void Vec3::UseDefaultThreadStorage() { (void)storage(); }
void Vec3::ReleaseThreadStorage() { tls.reset(); }
