...
private:
	OutputStream *stream;
    OutputStream& child(){if(!stream)throw IoError("Output stream has no child");return *stream;}
    void addWritten(unsigned int n){written=static_cast<int>(min<std::uint64_t>(std::uint64_t(written)+n,std::numeric_limits<int>::max()));}
