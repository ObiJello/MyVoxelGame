...
ByteArrayInputStream::ByteArrayInputStream(byteArray buf, unsigned int offset, unsigned int length)
	: count(0), mark(offset), pos(offset)
{
	validateByteRange(buf,offset,0);
	if(buf.length && !buf.data)throw std::invalid_argument("Input stream buffer is null");
	count=offset+min(length,buf.length-offset);
	this->buf = buf;
}
...
int ByteArrayInputStream::read(byteArray b, unsigned int offset, unsigned int length)
{
	...
	int k = static_cast<int>(min(min(length,count-pos),static_cast<unsigned>(std::numeric_limits<int>::max())));
	std::memmove(b.data+offset,buf.data+pos,k);
...
{
	if(n<=0)return 0;
	unsigned skipped=static_cast<unsigned>(min<__int64>(n,count-pos));
	pos+=skipped;
	return skipped;
}
