...
CompoundTag *NbtIo::decompress(byteArray buffer)
{
    ...
    struct DetachBorrowedBuffer {ByteArrayInputStream& stream; ~DetachBorrowedBuffer(){stream.reset();}} detach{bais};
	// 4J - this was using a try/finally block
...
byteArray NbtIo::compress(CompoundTag *tag)
{
	...
	byteArray ret=baos.toByteArray();
	dos.close();
	return ret;
}
...
CompoundTag *NbtIo::read(DataInput *dis)
{
	...
	if( tag->getId() == Tag::TAG_Compound ) return static_cast<CompoundTag *>(tag.release());

	// The owning temporary also releases a non-compound root.
