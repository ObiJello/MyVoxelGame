...
intArray VoronoiZoom::getArea(int xo, int yo, int w, int h)
{
	...
	for (int y = 0; y < ph - 1; y++)
	{
		...
		{
			double s = ss * 0.9;
			initRandom((static_cast<__int64>(x + px) * (1 << bits)), (static_cast<__int64>(y + py) * (1 << bits)));
			double x0 = (nextRandom(1024) / 1024.0 - 0.5) * s;
			double y0 = (nextRandom(1024) / 1024.0 - 0.5) * s;
			initRandom((static_cast<__int64>(x + px + 1) * (1 << bits)), (static_cast<__int64>(y + py) * (1 << bits)));
			double x1 = (nextRandom(1024) / 1024.0 - 0.5) * s + ss;
			double y1 = (nextRandom(1024) / 1024.0 - 0.5) * s;
			initRandom((static_cast<__int64>(x + px) * (1 << bits)), (static_cast<__int64>(y + py + 1) * (1 << bits)));
			double x2 = (nextRandom(1024) / 1024.0 - 0.5) * s;
			double y2 = (nextRandom(1024) / 1024.0 - 0.5) * s + ss;
			initRandom((static_cast<__int64>(x + px + 1) * (1 << bits)), (static_cast<__int64>(y + py + 1) * (1 << bits)));
