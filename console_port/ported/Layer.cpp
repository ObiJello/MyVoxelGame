...
int Layer::nextRandom(int max)
{
	...
	// "result = (int) ((std::bit_cast<std::int64_t>(rval) >> 24) % max);" is done twice in a row 'result' will not be the same. Need to speak to Sony about that
	...
	int result = (int) ((std::bit_cast<std::int64_t>(rval) >> 24) % max);
#endif
