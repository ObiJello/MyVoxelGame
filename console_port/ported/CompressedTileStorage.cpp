// CPU packing adapted from the preserved console source.
#include "stdafx.h"
...
{
    if(!copyFrom)throw IoError("Null compressed chunk copy");
	std::lock_guard storageGuard(cs_write);
...
{
    if(initFrom.data)codecTiles(initFrom,initOffset,32768);
	indicesAndData = NULL;
	allocatedSize = 0;
...
{
    if(y<0 || y>=128 || y%16)throw IoError("Invalid render section");std::lock_guard guard(cs_write);if(!indicesAndData)return true;
	int block;
		...
		for( int z = 0; z < 16; z += 4 )
	{
		...
		std::uint64_t comp;std::memcpy(&comp,&blockIndices[block],sizeof(comp));
...
{
    codecTiles(dataIn,inOffset,32768);
...
//	static int type0 = 0, type1 = 0, type2 = 0, type4 = 0, type8 = 0, chunkTotal = 0;
	...
	for( int i = 0; i < 512; i++ )
	{
		...
		std::uint64_t i64_1 = 1;	// MGH - instead of 1i64, which is MS specific
		for( int j = 0; j < 64; j++ )			// This loop of 64 is to go round the 4 x 4 tiles in the block
		{
			int tile = data[getIndex(i,j)];
...
{
    codecPosition(x,y,z);std::lock_guard guard(cs_write);if(!indicesAndData)return 0;
...
{
    codecPosition(x,y,z);if(val<0 || val>=255)throw IoError("Invalid tile value");
...
{
    unsigned countToCopy=codecRegion(dataIn,x0,y0,z0,x1,y1,z1,offset);
    codecTiles(dataIn,static_cast<unsigned>(offset),countToCopy);
    if(!countToCopy)return 0;
...
{
    unsigned countToCopy=codecRegion(dataInOut,x0,y0,z0,x1,y1,z1,offset);
    if(!countToCopy)return 0;
...
{
    std::lock_guard outerGuard(cs_write);if(upgradeBlock< -1 || upgradeBlock>=512)throw IoError("Invalid palette upgrade block");if(!indicesAndData)return;
...
{
    std::lock_guard guard(cs_write);if(!count0 || !count1 || !count2 || !count4 || !count8)throw IoError("Null palette statistics");
...
{
    std::lock_guard guard(cs_write);if(!indicesAndData)return -1;
	unsigned short *blockIndices = (unsigned short *)indicesAndData;
