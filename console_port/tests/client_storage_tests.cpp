...
int main(int argc,char** argv){try{
    ...
    auto survival=PS3WorldStorage::readFile(path);auto survivalData=survival->metadata();survivalData->setGameType(GameType::SURVIVAL);survival->putMetadata(*survivalData);survival->writeFile(invalidPath);rejects([&]{retained.load(invalidPath);});require(retained.revision==revision,"Unported survival saves cannot be opened as the creative client");
